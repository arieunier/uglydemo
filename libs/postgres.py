...
def __getSFUsers():
    ...
    if ((tmp_dict == None) or (tmp_dict == '')):
        ...
        rediscache.__setCache(key, json.dumps(data, default = myconverter), 60)
    else:
...
def __getImageAnalysis(tableName, objectName):
    ...
    if ((tmp_dict == None) or (tmp_dict == '')):
        ...
        rediscache.__setCache(key, json.dumps(data, default = myconverter), 120)
    else:
        logger.info("Data found in redis, using it directly")
        data = ujson.loads(tmp_dict)
...
def __getObjectsDescribe(tableName):
    ...
    if ((tmp_dict == None) or (tmp_dict == '')):
        ...
        logger.info(data)
        rediscache.__setCache(key, json.dumps(data, default = myconverter), 30)
    else:
        logger.info("Data found in redis, using it directly")
        data = ujson.loads(tmp_dict)
